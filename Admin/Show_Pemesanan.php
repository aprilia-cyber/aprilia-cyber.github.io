...
$null=mysql_query("select detail_pemesanan.* from pemesanan RIGHT JOIN detail_pemesanan ON pemesanan.ID=detail_pemesanan.ID_Pemesanan where pemesanan.ID is null");
while($detail=mysql_fetch_array($null))
			{
...
$query=mysql_query("select pemesanan.ID,pegawai.Nama,pembeli.Nama,pemesanan.Tgl_Pesan,Status_Kirim,ID_Pengirim,Tgl_Kirim from pegawai,pembeli,pemesanan where pegawai.ID=pemesanan.ID_Pegawai and pembeli.ID=pemesanan.ID_Pembeli order by pemesanan.ID");
...
<td align="center"><a href="<?php echo"delete_pemesanan.php?id=$tampil[0]";?>" onclick="return confirm('Anda yakin ingin membatalkan pemesanan ini ?')"> Cancel </a></td>
</tr>
<?php
$i++;
}
